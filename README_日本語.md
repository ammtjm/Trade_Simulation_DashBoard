...
本アプリは、トレード戦略の考察において、"対象資産の価格、ポジションの保有・解消のタイミングとその時の数量を元に、トレード戦略に対する評価指標(シャープレシオ、リターン、ドローダウンなど)を計算する"シミュレーション部分は共通しており、これをインタラクティブかつ簡便に表示することを目的としております。他ライブラリとして、Backtrader、Zipline、Pyfolioなどがありますが、これらと比較して機能を絞ることで
より簡便かつ直感的な可視化を提供できていると考えております。また、tradingviewといったチャートツールとは異なり、インジケータ等を用いたいわゆるテクニカル分析に基づく
トレード戦略の検証だけでなく、手元のデータを用いたより複雑で高度なトレード戦略のモデルに対するバックテストを簡単に行うことも可能です。さらに、これらの指標をエクセルファイルに出力することで、より詳細な分析を行うことも可能です。
